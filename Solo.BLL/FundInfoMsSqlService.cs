...
namespace Solo.BLL
{
    public class FundInfoMsSqlService : IFundInfoService
    {
        ...
        public List<FundView> GetFundInfosByQuery(FundQuery fundQuery)
        {
            ...
            string sql = "select *,(ISNULL(BuyRate, 0)+ISNULL(ManagerFee, 0)+ISNULL(CustodyFee, 0)+ISNULL(SaleFee, 0)) as TotalFee,mf.Status,mf.Investment from FundInfos fi left join MyFunds mf on fi.FundNo = mf.FundNo left join MyScore ms on fi.FundNo=ms.FundNo ";
        ...
        public string SetFundMainQueryStr(FundQuery fundQuery)
        {
            ...
            {
                queryStr += "(ISNULL(BuyRate, 0)+ISNULL(ManagerFee, 0)+ISNULL(CustodyFee, 0)+ISNULL(SaleFee, 0))<=" + fundQuery.MaxTotalFee + " and ";
            }
            if (fundQuery.MinTotalFee != null)
            {
                queryStr += "(ISNULL(BuyRate, 0)+ISNULL(ManagerFee, 0)+ISNULL(CustodyFee, 0)+ISNULL(SaleFee, 0))>=" + fundQuery.MinTotalFee + " and ";
            }
        ...
        public string SetAnalysisMainQueryStr(AnalysisQuery analysisQuery)
        {
            ...
            {
                queryStr += "(ISNULL(BuyRate, 0)+ISNULL(ManagerFee, 0)+ISNULL(CustodyFee, 0)+ISNULL(SaleFee, 0))<=" + analysisQuery.MaxTotalFee + " and ";
            }
            if (analysisQuery.MinTotalFee != null)
            {
                queryStr += "(ISNULL(BuyRate, 0)+ISNULL(ManagerFee, 0)+ISNULL(CustodyFee, 0)+ISNULL(SaleFee, 0))>=" + analysisQuery.MinTotalFee + " and ";
            }
        ...
        public StrategyTitle GetStrategyTitle()
        {
            ...
            else
            {
                ...
                da = SqlHelper.ExecuteTable(sql, pars);
                if (da.Rows.Count > 0)
                {
                ...
                else
                {
                    ...
                    if (da.Rows.Count > 0)
                    {
                        ...
                        SqlParameter[] pars1 = {
                                   new SqlParameter("@R1day",SqlDbType.Real),
                                   new SqlParameter("@R1week", SqlDbType.Real),
                                   new SqlParameter("@R1month", SqlDbType.Real)
                                 };
                        pars1[0].Value = strategyTitle.AR1day;
                        pars1[1].Value = strategyTitle.AR7day;
                        pars1[2].Value = strategyTitle.AR1month;
                        if (SqlHelper.ExecuteNonQuery(updatestr, CommandType.Text, pars1) != 1)
                        {
                            throw new Exception();
                        }
                    }
